...
**Requires:** WordPress 5.6+, WooCommerce 3.5+, PHP 7.0+
...
For every shippable plugin code change, update release metadata before committing.
Do this as part of the same change unless the user explicitly says the change
is not being prepared for release.

1. **changelog.txt** - Add new version entry with changes
2. **automaticffl-for-woocommerce.php** - Update `Version:` header comment and `AFFL_VERSION` constant
3. **README.md** - Update stable tag/version number and changelog section if present
4. **claude.md** - Update "Current Version" and Recent Changes
5. **AGENTS.md** - Update "Current Version" and Recent Changes

Ensure all version references reflect the same version number for consistency.
...
- **v1.0.23:** Fixed classic checkout layouts where the FFL firearm notice could wrap between floated shipping first/last name fields
